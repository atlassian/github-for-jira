...
|**Read** and **write** access to deployments|If you want to see build and deployment information in Jira, the app will need read and write permissions for deployments. In order for the GitHub for Jira app to send the correct status (pending, in progress, successful etc) of a deployment to Jira we require the `state` property. This property is only available on the `deployment_status` event which occurs when a deployment is created. GitHub will only send data to Jira via the app for this webhook when **both read & write** permissions are enabled. <br><br> **Deployments:** Read and write deployment permissions are used for the following webhooks: <br> - deployment status|
