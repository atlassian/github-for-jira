...
If an error occurs during the backfilling process, the app will prompt you to retry the backfilling for the failed repositories without having to restart the entire backfill process, except for permission errors. You have to first resolve the permission errors before retrying the backfill process.
