...
describe("PUT /jira/connect/enterprise/app/:uuid", () => {
	...
	it("should return 202 when correct uuid and installation id are passed, with partial data", async () => {
		...
		expect(await restoredApp.getDecryptedWebhookSecret(installation.plainClientKey)).toEqual("newSecret");
		expect(await restoredApp.getDecryptedPrivateKey(installation.plainClientKey)).toEqual("privatekey");
		expect(await restoredApp.getDecryptedGitHubClientSecret(installation.plainClientKey)).toEqual("secret");
	});
