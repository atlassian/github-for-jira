...
export const JiraSyncPost = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
	...
	try {
		...
		if (shouldUseBackfillAlgoIncremental && isIncrementalBackfilling(subscription, syncType, commitsFromDate)) {
			await findOrStartSync(subscription, req.log, syncType, commitsFromDate, ["pull", "branch", "commit", "build", "deployment"], { trigger: "ui-backfill-button" });
		} else {
			await findOrStartSync(subscription, req.log, syncType, commitsFromDate, undefined, { trigger: "ui-backfill-button" });
		}
