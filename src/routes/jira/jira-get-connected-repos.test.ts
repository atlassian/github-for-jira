...
describe("JiraGetConnectedRepos", () => {
	...
	it("should call the error middleware", async () => {
		let nullRes= {
		status: jest.fn(() => res)
	} as unknown as Response;;
        await JiraGetConnectedRepos(req, nullRes, next);
	...
	it('should call RepoSyncState.countSubscriptionRepos() with the correct arguments', async () => {
		const countSubscriptionRepos = jest.spyOn(RepoSyncState, 'countSubscriptionRepos').mockResolvedValueOnce(15);
	...
	it('should call RepoSyncState.findAllFromSubscription() with the correct arguments', async () => {
		...
		const findAllFromSubscription = jest.spyOn(RepoSyncState, 'findAllFromSubscription').mockResolvedValueOnce(Promise.resolve(reposyncState));
		const subscription = { id: 1 };
		Subscription.findByPk = jest.fn().mockResolvedValueOnce(subscription);
