			...
			it("testing the redirect URL in GithubOAuthLoginGet middleware", async () => {
				await supertest(app)
					...
					.then((response) => {
						...
						const expectedUrlWithoutState = `https://github.com/login/oauth/authorize?client_id=${envVars.GITHUB_CLIENT_ID}&redirect_uri=${encodeURIComponent(redirectUrl)}`;
			...
			it("testing the redirect URL in GithubOAuthLoginGet middleware", async () => {
				await supertest(app)
					...
					.then((response) => {
						...
						const expectedUrlWithoutState = `${gheUrl}/login/oauth/authorize?client_id=${GITHUB_SERVER_CLIENT_ID}&redirect_uri=${encodeURIComponent(redirectUrl)}`;
						expect(resultUrlWithoutState).toEqual(expectedUrlWithoutState);
					});
			});
