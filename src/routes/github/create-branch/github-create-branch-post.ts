...
export const GithubCreateBranchPost = async (req: Request, res: Response): Promise<void> => {
	...
	if (!owner || !repo || !sourceBranchName || !newBranchName) {
		res.status(400).send(getErrorMessages(400));
		return;
	}
	...
	try {
		...
		if (err.status === 403) {
			...
			res.status(err.status).send(getErrorMessages(err?.status, gitHubConfigurationLink));

		} else {
			res.status(err.status || 500).send(getErrorMessages(err?.status || 500));
		}
