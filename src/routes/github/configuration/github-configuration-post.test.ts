...
describe("github-configuration-post", () => {
	...
	describe("cloud", () => {
		...
		it("returns 401 when user is not an admin", async () => {
			...
			when(verifyAdminPermsAndFinishInstallation).calledWith(
				"myToken", installation, undefined, subscription.gitHubInstallationId + 1, expect.anything(), expect.anything(), expect.anything()
	...
	describe("server", () => {
		...
		it("returns 401 when the user is NOT an admin", async () => {
			...
			when(verifyAdminPermsAndFinishInstallation).calledWith(
				"myToken", installation, gitHubServerApp.id, subscription.gitHubInstallationId + 1, expect.anything(), expect.anything(), expect.anything()
			).mockResolvedValue({ error: "not admin" });
