...
describe("Session GET", () => {
	...
	describe("Frontend", () => {
		...
		describe("Testing session", () => {
			it("should return session.hbs", () =>
				supertest(app)
					.get("/session/jira/atlassian-connect.json")
					.expect(200)
					.then(response => {
						expect(response.text.includes("Logging you into GitHub")).toBeTruthy();
			...
			it("should return error asking for redirect url", () =>
				supertest(app)
					...
					.expect(500)
					.then(response => {
						expect(response.text.includes("Missing redirect url for session")).toBeTruthy();
						expect(response.text.includes("Logging you into GitHub")).toBeFalsy();
						expect(response.text.includes("Redirecting to your GitHub Enterprise Server instance")).toBeFalsy();
						expect(response.text.includes("Redirecting to your GitHub Cloud instance")).toBeFalsy();
					}));
		});
		...
		describe("Testing session for Cloud", () => {
			it("should return ghe-loading.hbs", () =>
				supertest(app)
					.get("/session/cloud/jira/atlassian-connect.json")
					.expect(200)
					.then(response => {
						expect(response.text.includes("Logging you into GitHub")).toBeFalsy();
						expect(response.text.includes("Redirecting to your GitHub Enterprise Server instance")).toBeFalsy();
