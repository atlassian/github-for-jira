...
export const getJiraClient = async (
	...
	const client = {
		...
		issues: {
			...
			): Promise<AxiosResponse<JiraIssue>> =>
			...
			getAll: async (
				...
				const responses = await Promise.all<
					AxiosResponse<JiraIssue> | undefined
				>(
		...
		devinfo: {
			...
			installation: {
				exists: (gitHubInstallationId: string | number) =>
					instance.get(`/rest/devinfo/0.10/existsByProperties`, {
						params: {
				...
				delete: async (gitHubInstallationId: string | number) =>
					Promise.all([
						...
						instance.delete("/rest/builds/0.1/bulkByProperties", {
							params: {
						...
						instance.delete("/rest/deployments/0.1/bulkByProperties", {
							params: {
								gitHubInstallationId,
							},
						}),
		...
		deployment: {
			submit: async (data): Promise<DeploymentsResult> => {
				...
				logger?.debug(
					`Sending deployments payload to jira. Payload: ${payload}`
				);
 ...
 */
const withinIssueKeyAssociationsLimit = (
	resources: JiraRemoteLink[]
): boolean => {
	if (!resources) {
		return true;
	}

	const issueKeyCounts = resources
		.filter((resource) => resource.associations?.length > 0)
		.map((resource) => resource.associations[0].values.length);
	return Math.max(...issueKeyCounts) <= ISSUE_KEY_API_LIMIT;
};
...
export const getTruncatedIssuekeys = (
	data: IssueKeyObject[] = []
): IssueKeyObject[] =>
...
const updateIssueKeyAssociationValuesFor = (
	...
	resources?.forEach((resource) => {
		if (resource.associations?.length > 0) {
			resource.associations[0].values = mutatingFunc(
				resource.associations[0].values
			);
		}
	});
