...
// } from "~/src/github/client/github-client.types";
...
export const getJiraClient = async (
	...
	class JiraClientNew {
		axios: AxiosInstance;
		baseURL: string;
		gitHubProduct: "server" | "cloud";


		// TODO, is this the best name?
		static async getNewClient(jiraHost: string, log: Logger) {
			const jiraClient = new JiraClientNew();
			jiraClient.baseURL = jiraHost;
			jiraClient.gitHubProduct = getCloudOrServerFromGitHubAppId(gitHubAppId);
			const installation = await Installation.getForHost(jiraHost);
		...
		public async parseIssues(issueIds: string[], query?: { fields: string }): Promise<JiraIssue[]> {
			const responses = await Promise.all<AxiosResponse<JiraIssue> | undefined>(
				issueIds.map((issueId) => this.getIssue(issueId, query)
					.catch(() => undefined)) // Ignore any errors
			);
			return responses.reduce((acc: JiraIssue[], response) => {
				if (response?.status === 200 && !!response?.data) {
					acc.push(response.data);
		...
		public parseIssue(text: string): string[] | undefined {
			if (!text) return undefined;
			return jiraIssueKeyParser(text);
		}

		public async listIssueComments(issue_id: string) {
			this.axios.get("/rest/api/latest/issue/{issue_id}/comment?expand=properties", {
				urlParams: {
					issue_id
				}
			});
		...
		public async addIssueComment(issue_id: string, payload) {
			this.axios.post("/rest/api/latest/issue/{issue_id}/comment", payload, {
				urlParams: {
					issue_id
				}
			});
		...
		public async updateIssueComment(issue_id: string, comment_id: string, payload) {
			this.axios.put("rest/api/latest/issue/{issue_id}/comment/{comment_id}", payload, {
				urlParams: {
					issue_id,
					comment_id
				}
		...
		public async deleteIssueComment(issue_id: string, comment_id: string) {
			this.axios.delete("rest/api/latest/issue/{issue_id}/comment/{comment_id}", {
				urlParams: {
					issue_id,
					comment_id
				}
		...
		public async getIssueTransitions(issue_id: string) {
			this.axios.get("/rest/api/latest/issue/{issue_id}/transitions", {
				urlParams: {
					issue_id
				}
			});
		...
		public async updateIssueTransistion(issue_id: string, transition_id: string) {
			this.axios.post("/rest/api/latest/issue/{issue_id}/transitions", {
				transition: {
					id: transition_id
				},
				urlParams: {
		...
		public async deleteBranch(transformedRepositoryId: TransformedRepositoryId, branchRef: string) {
			this.axios.delete("/rest/devinfo/0.10/repository/{transformedRepositoryId}/branch/{branchJiraId}", {
				params: {
