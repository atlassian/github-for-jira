  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      if (!data.viewer.organization) {
        req.log.info(
          `Non Atlassian scoped token attempted to access staff routes: login=${data.viewer.login}, isEmployee=${data.viewer.isEmployee}`,
        );
        res.status(401).json({
          error: 'Unauthorized',
      ...
      req.log.info(
        `Staff routes accessed: login=${data.viewer.login}, isEmployee=${data.viewer.isEmployee}, viewerPermission=${data.viewer.organization.repository.viewerPermission}`,
      );
...
router.post(
  ...
  async (request: Request, response: Response): Promise<void> => {
    ...
    statsd.increment('resync_failed', failedSubscriptions.length, tags);
