...
describe.skip("GitHub Actions", () => {
	...
	describe("add to push queue", () => {
		...
		it("should add push event to the queue if Jira issue keys are present", async () => {
			const event = require("../../../../test-utils/fixtures/push-basic.json");
		...
		it("should not add push event to the queue if there are no Jira issue keys present", async () => {
			const event = require("../../../..//test-utils/fixtures/push-no-issues.json");
			await app.receive(event);
		});

		it("should handle payloads where only some commits have issue keys", async () => {
			const event = require("../../../../test-utils/fixtures/push-mixed.json");
			await app.receive(event);
	...
	describe("process push payloads", () => {
		...
		it("should update the Jira issue when no username is present", async () => {
			const event = require("../../../../test-utils/fixtures/push-no-username.json");
			const job = {
		...
		it("should only send 10 files if push contains more than 10 files changed", async () => {
			const event = require("../../../../test-utils/fixtures/push-multiple.json");
			const job = {
				data: createJobData(event.payload, process.env.ATLASSIAN_URL)
			};
			...
			githubNock
				...
				.replyWithFile(200, "../../../../test-utils/fixtures/more-than-10-files.json");
		...
		//   const fixture = require('../../../../test-utils/fixtures/push-comment.json')
		...
		//   const fixture = require('../../../../test-utils/fixtures/push-worklog.json')
		...
		//   const fixture = require('../../../../test-utils/fixtures/push-transition.json')
		...
		// it('should run a transition command in the commit message', async () => {
		//   const fixture = require('../../../../test-utils/fixtures/push-transition-comment.json')
		...
		//   const fixture = require('../../../../test-utils/fixtures/push-multiple.json')
		...
		it("should not run a command without a Jira issue", async () => {
			const fixture = require("../../../..//test-utils/fixtures/push-no-issues.json");
		...
		it("should support commits without smart commands", async () => {
			const fixture = require("../../../../test-utils/fixtures/push-empty.json");
		...
		it("should add the MERGE_COMMIT flag when a merge commit is made", async () => {
			const event = require("../../../../te/../test-utils/fixtures/push-no-username.json");
			const job = {
				data: createJobData(event.payload, process.env.ATLASSIAN_URL)
			};
		...
		it("should not add the MERGE_COMMIT flag when a commit is not a merge commit", async () => {
			...
			githubNock.get("/repos/test-repo-owner/test-repo-name/commits/commit-no-username")
				.replyWithFile(200, "../../../../test-utils/fixtures/push-non-merge-commit");
