...
app.use(async (req, res, next) => {
  ...
  try {
    ...
    if (errors) {
      logger.error(`Permissions error: ${errors}`);
      res.status(401);
      return res.json({ errors, viewerPermissionQuery });
    }

    if (!data.viewer.organization) {
      logger.error(`Non Fusion-Arc scoped token attempted to access staff routes: login=${data.viewer.login}, isEmployee=${data.viewer.isEmployee}`);

      res.status(401);
      return res.json({
        error: 'Unauthorized',
        message: 'Token provided does not have access to the `fusion-arc` organization',
      });
    }
    ...
    logger.info(`Staff routes accessed: login=${data.viewer.login}, isEmployee=${data.viewer.isEmployee}, viewerPermission=${data.viewer.organization.repository.viewerPermission}`);
...
app.get('/:installationId', [
  ...
  try {
    ...
    const failedConnections = installations.filter(response => {
      logger.error(`Connection failed: ${response.error}`);
    ...
    logger.error(`Failed to get :installationId - ${err}`);
    res.status(500);
    return res.json(err);
  }
});
...
app.get('/:installationId/repoSyncState.json', [
  ...
  try {
    ...
    logger.error(`Failed to get :installationId/repoSyncState.json - ${err}`);
    res.status(500);
    return res.json(err);
  }
});
...
app.post('/:installationId/sync', [
  ...
  try {
    logger.info(jiraHost, installationId);
    const subscription = await Subscription.getSingleInstallation(jiraHost, installationId);
    ...
    logger.info('Successfully (re)started sync.');
    ...
    logger.error(`Sync restart error: ${err}`);
...
app.post('/resyncFailed', bodyParser, async (request, response) => {
  try {
    ...
    logger.error(`resyncFailed error: ${err}`);
  }
});
...
app.get('/jira/:clientKeyOrJiraHost', [
  ...
  try {
    ...
    if (jiraInstallations.length > 0) {
      const data = jiraInstallations.map((jiraInstallation) => serializeJiraInstallation(jiraInstallation, logger));
      response.json(await Promise.all(data));
      logger.info('Found Jira installation.');
    } else {
      response.sendStatus(404);
      logger.error('No Jira installation found.');
    }
  } catch (err) {
    logger.error(`Error in jira/:clientKeyOrJiraHost: ${err}`);
  }
});
...
app.post('/jira/:clientKey/uninstall', [
  ...
  try {
    if (response.locals.installation) {
      const jiraClient = new JiraClient(response.locals.installation, logger);
      const checkAuthorization = request.body.force !== 'true';

      if (checkAuthorization && await jiraClient.isAuthorized()) {
        logger.error('Refusing to uninstall authorized Jira installation');
        response.status(400).json({ message: 'Refusing to uninstall authorized Jira installation' });
      } else {
        logger.info(`Forcing uninstall for ${response.locals.installation.clientKey}`);

        const uninstall = require('../jira/uninstall');
        await uninstall(request, response);
      }
    } else {
      logger.error('Error jira/:clientKey/uninstall');
      response.sendStatus(404);
    }
  } catch (err) {
    logger.error(`Error in jira/:clientKey/uninstall: ${err}`);
  }
});
...
app.post('/jira/:installationId/verify', [
  ...
  try {
    ...
    if (installation.enabled) {
      ...
      await verifyInstallation(installation, logger)();
    ...
    logger.error(`Error in jira/:installationId/verify: ${err}`);
  }
});
...
app.post('/:installationId/migrate/:undo?', [
  ...
  const jiraClient = await getJiraClient(jiraHost, installationId);

  if (req.params.undo) {
    try {
      await jiraClient.devinfo.migration.undo();

      logger.info('Successfully called migrationUndo');
      ...
      logger.error(`Error trying to undo migration: ${err}`);
    ...
    try {
      ...
      logger.info('Successfully called migrationComplete');
      res.send('Successfully called migrationComplete');

      await subscription.update({ syncStatus: 'COMPLETE' });
      return;
    } catch (err) {
      logger.error(`Error trying to complete migration: ${err}`);
