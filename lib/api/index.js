...
const isProd = require('../jira/util/isProd');
...
app.post('/:installationId/migrate/:undo?', [
  ...
  try {
    if (req.params.undo) {
      ...
      const responseMsg = isProd() ? 'Successfully called migrationUndo' : 'Sync failed';
      ...
      const responseMsg = isProd() ? 'Successfully called migrationComplete' : 'Sync complete';
      res.send(responseMsg);
