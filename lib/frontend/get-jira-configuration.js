...
module.exports = async (req, res) => {
  ...
  const totalNumberOfRepos = await Promise.all(subscriptions
    .map(subscription => Object.keys(subscription.repoSyncState.repos).length));

  const numberOfSyncedRepos = await Promise.all(subscriptions
    .map(subscription => subscription.numberOfSyncedRepos || 0));
  ...
  try {
    res.render('jira-configuration.hbs', {
      ...
      totalNumberOfRepos: totalNumberOfRepos[0],
      numberOfSyncedRepos,
    });
    return 'Jira configuration rendered successfully.';
  } catch (error) {
    console.error(error);
    return `Failed to render Jira configuration: ${error}`;
  }
};
