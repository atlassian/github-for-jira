...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!userInstallation) {
      logger.error('Failed to add subscription. User does not have access to that installation.');
    ...
    if (userInstallation.target_type === 'Organization') {
      ...
      const { data: { role } } = await res.locals.github.orgs.getMembership({ org: userInstallation.account.login, username: login });

      if (role !== 'admin') {
        logger.error('Failed to add subscription. User is not an admin of that installation.');
    ...
    logger.error(`Failed to POST GitHub config: ${err}`);
    res.sendStatus(400);
  }
};
