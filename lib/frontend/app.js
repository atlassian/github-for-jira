...
module.exports = (appTokenGenerator) => {
  ...
  const sentryMiddleware = async (err, req, res, next) => {
    Sentry.withScope(async scope => {
      scope.setTag('jiraHost', req.session.jiraHost || req.query.xdm_e)

      if (req.session.githubToken) {
        try {
          const { data: { login } } = (await res.locals.github.users.get())
          Sentry.setUser({ login })
        } catch (err) {
          req.log.error(err, 'unable to get user info')
        }
      }
