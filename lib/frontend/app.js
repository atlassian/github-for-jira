...
module.exports = (appTokenGenerator) => {
  ...
  app.use(['/github/setup', '/github/configuration'], (req, res, next) => {
  ...
  oauth.on('token', function (token, res, tokenRes, req) {
    ...
    if (req.session.jiraHost) {
      route = (`${req.session.jiraHost}/plugins/servlet/upm/marketplace/plugins/com.github.integration.production`)
    } else if (req.session.queryParams) {
