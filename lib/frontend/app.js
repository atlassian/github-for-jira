...
module.exports = (appTokenGenerator) => {
  ...
  app.use(session({
    ...
    signed: true,
    // TODO: Should we make sameSite strict? Not sure if none is appropriate
    sameSite: 'none',
    secure: true,
  }));
  ...
  app.use(['/github/setup', '/github/login', '/github/configuration', '/github/installations'], (req, res, next) => {
  ...
  oauth.on('token', (token, res, tokenRes, req) => {
    ...
    return res.redirect(res.locals.redirect || '/github/configure');
  });
