...
const { checkGithubAuth } = require('./github-oauth');
...
module.exports = (appTokenGenerator) => {
  ...
  app.get('/github/setup', csrfProtection, checkGithubAuth, getGitHubSetup);
  app.post('/github/setup', csrfProtection, checkGithubAuth, postGitHubSetup);

  app.get('/github/configuration', csrfProtection, checkGithubAuth, getGitHubConfiguration);
  app.post('/github/configuration', csrfProtection, checkGithubAuth, postGitHubConfiguration);

  app.get('/github/installations', csrfProtection, checkGithubAuth, listGitHubInstallations);
  ...
  oauth.on('token', (token, res, tokenRes, req) => {
    ...
    return res.redirect(res.locals.redirect || '/github/configuration');
  });
