...
module.exports = function (opts) {
  ...
  function callback(req, res, next) {
    ...
    if (!state || !redirectUrl) {
      logger.error('ERROR: Missing matching Auth state parameter');
      return next(new Error('Missing matching Auth state parameter'));
    }

    if (!code) {
      logger.error('ERROR: Missing OAuth Code');
    ...
    request.get({
      ...
      if (err) {
        logger.error('ERROR: Cannot retrieve access token from Github');
