...
const processInstallation = (queues) => {
  const updateJobStatus = async (jiraClient, job, edges, task, repositoryId, app) => {
    ...
    if (subscription == null) {
      app.log.info('Organization has been deleted. Other active syncs will continue.');
      return;
    }

    const status = edges.length > 0 ? 'pending' : 'complete';
    app.log.info(`Updating job status for installationId=${installationId}, repositoryId=${repositoryId}, task=${task}, status=${status}`);
    ...
    if (edges.length > 0) {
      ...
      if (!getNextTask(subscription)) {
        ...
        app.log.info(`Sync status for installationId=${installationId} is pending`);
        const { removeOnComplete, removeOnFail } = job.opts;
