...
const processInstallation = (app, queues) => {
  const updateJobStatus = async (jiraClient, job, edges, task, repositoryId) => {
    ...
    console.log('LOGGER here: ', logger);
  ...
  return async function (job) {
    ...
    try {
      ...
      if (delay) { // if not NaN or 0
        logger.error(`Delaying job for ${delay}ms installationId=${installationId}, repositoryId=${repositoryId}, task=${task}`);
        const { removeOnComplete, removeOnFail } = job.opts;
      ...
      if (String(err).includes('connect ETIMEDOUT')) {
        ...
        logger.error(`ETIMEDOUT error, retrying in 5 seconds: installationId=${installationId}, repositoryId=${repositoryId}, task=${task}`);
        const { removeOnComplete, removeOnFail } = job.opts;
      ...
      if (String(err.message).includes('You have triggered an abuse detection mechanism')) {
        ...
        logger.error(`Abuse detection triggered. Retrying in 60 seconds: installationId=${installationId}, repositoryId=${repositoryId}, task=${task}`);
        const { removeOnComplete, removeOnFail } = job.opts;
      ...
      if (isNotFoundError) {
        logger.error(`Repository deleted after discovery, skipping initial sync: installationId=${installationId}, repositoryId=${repositoryId}, task=${task}`);
