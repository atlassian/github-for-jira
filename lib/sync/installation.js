...
const updateNumberOfReposSynced = async (repos, subscription) => {
  ...
  try {
    ...
    logger.error(`Error updating repoSyncState: ${err}`);
  }
};
 ...
 */
const processInstallation = (app, queues) => {
  ...
  async function getEnhancedGitHub(installationId) {
    try {
      ...
      logger.error(`Error enhancing GitHub: ${err}`);
    }
  }
  ...
  return async function (job) {
    ...
    const jiraClient = await getJiraClient(subscription.jiraHost, installationId);
    ...
    try {
      ...
      if (delay) { // if not NaN or 0
        logger.info(`Delaying job for ${delay}ms installationId=${installationId}, repositoryId=${repositoryId}, task=${task}`);
        const { removeOnComplete, removeOnFail } = job.opts;
      ...
      if (String(err).includes('connect ETIMEDOUT')) {
        ...
        logger.info(`ETIMEDOUT error, retrying in 5 seconds: installationId=${installationId}, repositoryId=${repositoryId}, task=${task}`);
        const { removeOnComplete, removeOnFail } = job.opts;
      ...
      if (String(err.message).includes('You have triggered an abuse detection mechanism')) {
        ...
        logger.info(`Abuse detection triggered. Retrying in 60 seconds: installationId=${installationId}, repositoryId=${repositoryId}, task=${task}`);
        const { removeOnComplete, removeOnFail } = job.opts;
      ...
      logger.error(`ERROR: Sync failed: ${err}`);
