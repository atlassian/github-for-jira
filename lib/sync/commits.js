  ...
  return async function (job) {
    ...
    while (cursor !== undefined) {
      ...
      if (!data && edges.length > 0) {
        console.log('ssuuuuuuup', subscription.get('repoSyncState').repos)
        // robot.log(`No Jira issue found on commits for this page of results`)
        cursor = edges[edges.length - 1].cursor
        await subscription.set('reposyncstate.repos', 'test1')
        await subscription.save()
        continue
      } else if (!data) {
        await subscription.set('repoSyncState.repos', 'test2')
        await subscription.save()
        cursor = undefined
        return
      }
      ...
      if (edges.length !== 0) {
        cursor = edges[edges.length - 1].cursor
        await subscription.set('repoSyncState.repos', 'test3')
        await subscription.save()
      } else {
        await subscription.set('repoSyncState.repos', 'test4')
        await subscription.save()
        cursor = undefined
      }
    }
