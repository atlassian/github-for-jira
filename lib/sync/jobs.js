...
const getPendingRepos = subscription =>
  Object.entries(subscription.repoSyncState.repos)
...
exports.startJob = async (app, job, jobType) => {
  ...
  const { repos: repoSyncState } = subscription.repoSyncState
...
exports.updateJobStatus = async (app, queue, job, subscription, edges, jobType) => {
  const { installationId, repository } = job.data
  const status = edges.length > 0 ? 'pending' : 'complete'
  app.log(`Updating status job for ${jobInfo(installationId, repository, jobType)}, status=${status}`)
  ...
  if (edges.length > 0) {
    ...
    app.log('pendingRepos', JSON.stringify(pendingRepos, null, 2))
