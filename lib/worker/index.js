...
module.exports = {
  ...
  start() {
    queues.discovery.process('discovery', 5, commonMiddleware(discovery(app, queues)));
    queues.installation.process('installation', Number(CONCURRENT_WORKERS), commonMiddleware(processInstallation(app, queues)));
    queues.push.process('push', Number(CONCURRENT_WORKERS), commonMiddleware(processPush(app)));
    queues.metrics.process('metrics', 1, commonMiddleware(metricsJob));
