...
const queueOpts = {
  redis: process.env.REDIS_BOTTLENECK_HOST,
  defaultJobOptions: {
...
Object.keys(queues).forEach(name => {
  ...
  // ! ERROR HERE e.g.
  // {"name":"github-for-jira","hostname":"dd1e52b5a156","pid":18,"level":"ERROR","msg":"Error occurred while processing queue discovery: Error: connect ECONNREFUSED 127.0.0.1:6379","time":"2021-05-31T11:56:41.350Z","v":0}
  ...
  return async (job) => {
    newrelic.startBackgroundTransaction(`job ${job.queue.name}`, 'worker queue', async () => {
      ...
      try {
        ...
        logger.error(`New Relic middleware error: ${err}`);
      } finally {
        transaction.end();
      }
    });
