...
const clientRedisOptions = getRedisInfo('client').redisOptions;
logger.info(`CLIENT REDIS OPTIONS: ${clientRedisOptions}`);
const client = new Redis(clientRedisOptions);

const subscriberRedisOptions = getRedisInfo('subscriber').redisOptions;
logger.info(`SUBSCRIBER REDIS OPTIONS: ${subscriberRedisOptions}`);
const subscriber = new Redis(subscriberRedisOptions);
  ...
  createClient: (type, redisOpts = {}) => {
    ...
    switch (type) {
      ...
      case 'subscriber':
        return subscriber;
      default:
        redisInfo = Object.assign(getRedisInfo('bclient'), redisOpts);
