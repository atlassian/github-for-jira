...
const setupDeepcheck = require('./deepcheck');
...
async function getGitHubCIDRs() {
  try {
    ...
    logger.error(`Error in getGitHubCIDRs: ${err}`);
  }
}
...
module.exports = async (app) => {
  try {
    if (process.env.USE_RATE_LIMITING === 'true') {
      const GitHubCIDRs = await getGitHubCIDRs();
    ...
    logger.error(`Error configuring robot: ${err}`);
  }
};
