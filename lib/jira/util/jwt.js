...
function extractJwtFromRequest(req) {
  ...
  if (tokenInQuery && tokenInBody) {
    logger.error('JWT token can only appear in either query parameter or request body.');
    return;
  }
  ...
  if (authHeader && authHeader.startsWith('JWT ')) {
    if (token) {
      ...
      logger.info(`JWT token found in ${foundIn} and in header: using ${foundIn} value.`);
    } else {
