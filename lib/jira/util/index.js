...
module.exports = function (jiraClient) {
  function addJiraIssueLinks(text, issues) {
    ...
    const referencedLinksRegex = /(http|https):\/\/[a-zA-Z0-9\-.]+\.[a-zA-Z]{2,3}(\/\S*)?/;
    const previouslyReferenced = referencedLinksRegex.exec(text);

    // if previously, then create the same object created by markdown.parse
    // const [, key] = previouslyReferenced;
    // console.log();
