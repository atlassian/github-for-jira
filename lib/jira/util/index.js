...
module.exports = (jiraClient) => {
  function addJiraIssueLinks(text, issues) {
    ...
    const issueMap = issues.reduce((iMap, issue) => ({
      ...iMap,
      [issue.key]: issue,
    }), {});
  ...
  async function runJiraCommands(commands) {
    return Promise.all(commands.map(command => {
      if (command.kind === 'comment') {
        return Promise.all(
          command.issueKeys.map(
            issueKey => jiraClient.issues.comments.addForIssue(issueKey, { body: command.text }),
          ),
        );
      }
      ...
      if (command.kind === 'worklog') {
        return Promise.all(
          command.issueKeys.map(
            issueKey => jiraClient.issues.worklogs.addForIssue(issueKey, {
