    ...
    devinfo: {
      ...
      repository: {
        ...
        update: async (data, options) => {
          ...
          if (data.commits && data.commits.length >= 400) {
            await batchedCommitRequest(data, options, instance, gitHubInstallationId)
          } else {
 ...
 */
const batchedCommitRequest = async (data, options, instance, installationId) => {
  const filteredCommits = dedupCommits(data)

  const commitChunks = []
  while (filteredCommits.length) {
    commitChunks.push(filteredCommits.splice(0, 400))
  }

  const batchedUpdates = []
  commitChunks.forEach((commitChunk) => {
    const repositoryObj = {
      ...data,
      commits: commitChunk
    }
 ...
 */
const dedupCommits = (repositoryObj) => {
  if (!repositoryObj.commits) return []

  return repositoryObj.commits.filter((obj, pos, arr) => {
    return arr.map(mapCommit => mapCommit.id).indexOf(obj.id) === pos
  })
}
