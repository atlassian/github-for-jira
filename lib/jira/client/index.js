...
const issueKeySet = (resources) => {
  ...
  let issueKeys = []
  resources.map(resource => issueKeys = issueKeys.concat(resource.issueKeys))
    ...
    devinfo: {
      ...
      repository: {
        ...
        update: (data, options) => {
          ...
          if (issueKeySet(data.commits).length <= 400 && issueKeySet(data.branches).length <= 400) {
            ...
            // We have more than 400 issue keys so raise an exception
