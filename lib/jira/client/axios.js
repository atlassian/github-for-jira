...
const instrumentRequest = (response) => {
  ...
  logger.info(`response status from Jira: ${response.status}`);
...
const instrumentFailedRequest = (error) => {
  if (error.response) {
    ...
    logger.error('Error during Axios request has no response property:', error);
  }
