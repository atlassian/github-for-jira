...
module.exports = async (context, jiraClient, util) => {
  ...
  const commits = await context.github.pulls.listCommits({ owner: context.payload.repository.owner.login, repo: context.payload.repository.name, pull_number: context.payload.pull_request.number });
  const { data: jiraPayload } = transformPullRequest(context.payload, author.data, commits);
