...
module.exports = async (context, jiraClient, util) => {
  const author = await context.octokit.users.getByUsername({ username: context.payload.pull_request.user.login });
  let reviews = {};
  try {
    reviews = await context.octokit.pulls.listReviews({
  ...
  if (linkifiedBody) {
    ...
    await context.octokit.issues.update(editedPullRequest);
  }
