...
module.exports = function middleware (callback) {
  return withSentry(async (context) => {
    ...
    if (context.payload.sender.type === 'Bot') {
      statsd.increment('noop', { type: 'bot' })
      context.log( { noop: 'bot' }, 'Halting futher execution since the sender is a bot')
      return
    }

    if (isFromIgnoredRepo(context.payload)) {
      statsd.increment('noop', { type: 'ignored_repo' })
      context.log( { noop: 'ignored_repo', installation_id: payload.installation.id, repository_id: payload.repository.id }, 'Halting futher execution since the repository is explicilty ignored')
      return
    }

    const subscriptions = await Subscription.getAllForInstallation(gitHubInstallationId)
    if (!subscriptions.length) {
      statsd.increment('noop', { type: 'no_subscriptions' })
    ...
    for (let subscription of subscriptions) {
      ...
      if (jiraClient == null) {
        ...
        statsd.increment('noop', { type: 'no_jira_client' })
