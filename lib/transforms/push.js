...
async function enqueuePush(payload, jiraHost) {
  ...
  try {
    logger.info(`adding 'push' job to queue: ${JSON.stringify(jobData)}`);
    await queues.push.add(
      jobData,
      jobOpts,
    );
  } catch (err) {
    logger.error(`enqueuePush error: ${err}`);
  }
}
...
function processPush(app) {
  return async function (job) {
    logger.info(`processing 'push' job: ${JSON.stringify(job.data)}`);
    try {
      const {
        repository,
      ...
      logger.info(`found subscription for 'push' job: ${subscription.gitHubInstallationId}`);
      ...
      for (const chunk of chunks) {
        ...
        logger.info(`sending data to Jira: ${JSON.stringify(jiraPayload)}`);
