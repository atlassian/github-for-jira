...
async function enqueuePush (payload, jiraHost) {
  ...
  for (const commit of payload.commits) {
    ...
    // The job only needs the sha. All other data will be requested from GitHub as part of the job
    // Creating an array of shas for the job processor to work on
...
function processPush (app) {
  return async function (job) {
    ...
    const commits = shas.forEach(async sha => {
      const { data, data: { commit: githubCommit } } = await github.repos.getCommit({ owner, repo, sha: sha.id })
      const { files, author: { username } } = data
    ...
    // Jira accepts 100 objects per request
    // break the array up into chunks of 100
    const chunks = []
    while (commits.length) {
      chunks.push(commits.splice(0, 2))
    }
