...
function createJobData(payload, jiraHost) {
  ...
  payload.commits.forEach((commit) => {
    ...
    // Only store the sha and issue keys. All other data will be requested from
    // GitHub as part of the job Creates an array of shas for the job processor
    // to work on
    if (issueKeys) {
...
function processPush(app) {
  return async (job) => {
    ...
    await chunks.forEach(async (chunk) => {
      ...
      await projects.forEach(async (projectKey) => Project.upsert(projectKey, jiraClient.baseURL));
    });
  };
}
