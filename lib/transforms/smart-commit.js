const issueKeysRegex = /(?:^|\s)((?:(?:\[)?[A-Za-z][A-Za-z0-9_]+-[0-9]+(?:\])?\s*)+)/
...
function parseIssueKeys (text) {
  // Find issues
  const issueText = text.match(issueKeysRegex)

  if (!issueText) {
    return
  }

  const issues = issueText[0].trim()
    .split(/\s+/)
...
module.exports = (sourceText) => {
  ...
  return {
    issueKeys: parseIssueKeys(issueKeysText),
