...
module.exports = (source) => {
  ...
  const appendToCommandText = function (token) {
    if (!command.text) {
      if (token.type == 'comment') {
        command.text = token.value
      }
    } else {
      command.text = command.text + token.value
    }
  }
  ...
  const rulesByState = {
    ...
    transition: {
      transition,
      time,
      comment: {action: appendToCommandText},
      whitespace: {action: appendToCommandText},
      newline: {action: function() { /* no-op */ }, pop: true},
    },
    time: {
      transition,
