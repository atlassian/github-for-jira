...
module.exports = class Subscription extends Sequelize.Model {
    ...
    return super.init(
      {
        ...
        syncStatus: DataTypes.ENUM('PENDING', 'COMPLETE', 'IN PROGRESS', 'FAILED'),
  ...
  // A stalled in progress sync is one that is IN PROGRESS but has not seen any updates in the last 15 minutes
  // This may happen when an error causes a sync to die without setting the status to 'FAILED'
  isInProgressSyncStalled() {
    if (this.syncStatus === 'IN PROGRESS') {
