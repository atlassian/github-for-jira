...
module.exports = class Subscription extends Sequelize.Model {
  ...
  static async agedSyncCounts () {
    const ageThreshold = moment().subtract(7, 'd').format('YYYY-MM-DD')
    const [ results ] = await this.sequelize.query(
      `SELECT "syncStatus", COUNT(*) FROM "Subscriptions" WHERE "updatedAt" < :ageThreshold AND ("syncStatus" IS NULL OR "syncStatus" in ('ACTIVE', 'PENDING')) GROUP BY "syncStatus"`,
      {
        replacements: { ageThreshold }
      }
    )
    return results
  }
