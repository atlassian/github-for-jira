...
module.exports = class Subscription extends Sequelize.Model {
   ...
   *
   * @param {string} youngestUpdatedAt - REQUIRED An INTERVAL expression for the youngest updatedAt for running syncs
   * @param {string} oldestUpdatedAt - OPTIONAL An INTERVAL expression for the oldest updatedAt running syncs
   ...
   */
  static async agedSyncCounts (youngestUpdatedAt, oldestUpdatedAt) {
    var criteria;
    if (oldestUpdatedAt === undefined) {
      criteria = {
        clause: `"updatedAt" < NOW() - INTERVAL :youngestUpdatedAt`,
        replacements: { youngestUpdatedAt }
      }
    } else {
      criteria = {
        clause: `"updatedAt" < NOW() - INTERVAL :youngestUpdatedAt AND "updatedAt" >= NOW() - INTERVAL :oldestUpdatedAt`,
        replacements: { youngestUpdatedAt, oldestUpdatedAt }
      }
    }
